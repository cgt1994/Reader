 ...
 */
public class GetImgHandler implements AndServerRequestHandler {
    private String img;

    public GetImgHandler(String img) {
        this.img = img;
    }

    @Override
    public void handle(HttpRequest request, HttpResponse response, HttpContext context) throws HttpException, IOException {
        File file = new File(SPHelper.getUploadIndex(ReaderApplication.getInstance().getApplicationContext()).replace("index.html", "img/" + img));
        if (file.exists()) {
